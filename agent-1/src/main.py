...
def main():
   ...
   for loc_index, item in enumerate(locations, start=1):
       ...
       for hotel_index, hotel in enumerate(hotels, start=1):
           ...
           if isinstance(hotel_name, dict):
               ...
               hotel_name = ""
           # ----------------------------------------------------------
           # Franchise Detection (only for highly distressed hotels)
           # ----------------------------------------------------------
           franchise_result = {
               "was_franchise": False,
               "former_brand": "",
           ...
           if score >= 7:
               print(
                   "    [FRANCHISE] Checking historical franchise affiliation...",
                   flush=True
               )
               try:
                   ...
                   print(
                       f"    [FRANCHISE] "
                       f"was_franchise={franchise_result['was_franchise']} | "
                       f"former_brand={franchise_result['former_brand'] or 'N/A'} | "
                       f"confidence={franchise_result['franchise_confidence']}",
                       flush=True
                   )
               except Exception as e:
           ...
           # Add franchise information ONLY for priority hotels
           if score >= 7:
               all_rows[-1].update({
                   "was_franchise": franchise_result["was_franchise"],
                   "former_brand": franchise_result["former_brand"],
                   "franchise_confidence": franchise_result["franchise_confidence"],
                   "franchise_evidence": franchise_result["franchise_evidence"]
               })
   ...
   # ----------------------------------------------------------
   # Save ALL hotels (without franchise columns)
   # ----------------------------------------------------------
   ...
   # ----------------------------------------------------------
   # Save PRIORITY hotels (with franchise columns)
   # ----------------------------------------------------------
   priority_rows = [
